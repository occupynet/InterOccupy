		...
		jQuery.ajax({
			...
			success: function( data ) { // data contains json response objects.
				...
				if ( pb_step == '0' ) { // Finished. Stop polling.
					...
					jQuery('#pb_backupbuddy_loading').hide();
					jQuery( '#pb_backupbuddy_status' ).html( data.status_message );
...
<div class="pb_backupbuddy_start_migration">
	...
	Enter the URL corresponding to the <?php
	...
	<form id="pb_backupbuddy_migration_form">
		<span style="display: inline-block; width: 140px;">
			Destination site URL</span><input type="text" name="url" id="pb_backupbuddy_url" size="40" value="<?php echo $destination_url; ?>">
			<span class="description">Guess: <?php echo $guess_url; ?></span>
		</span>
		<br><br>
		<span style="display: inline-block; width: 140px;">
		...
		<span id="pb_backupbuddy_status"><?php _e( 'Status: Waiting on destination URL to be entered.', 'it-l10n-backupbuddy' ); ?></span>
		<span id="pb_backupbuddy_loading" style="display: none; margin-left: 10px;"><img src="<?php echo pb_backupbuddy::plugin_url(); ?>/images/loading.gif" <?php echo 'alt="', __('Loading...', 'it-l10n-backupbuddy' ),'" title="',__('Loading...', 'it-l10n-backupbuddy' ),'"';?> width="16" height="16" style="vertical-align: -3px;" /></span>
	</form>
