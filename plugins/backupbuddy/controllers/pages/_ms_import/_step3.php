...
if ( isset( $this->advanced_options['skip_files'] ) && ( $this->advanced_options['skip_files'] == 'true' ) ) {
	...
	if ( !isset( pb_backupbuddy::$classes['zipbuddy'] ) ) {
		pb_backupbuddy::$classes['zipbuddy'] = new pluginbuddy_zipbuddy( ABSPATH, '', 'unzip' );
	}
...
if ( true !== $result ) {
	...
	if ( !file_exists( $destination_directory . 'wp-config.php' ) ) {
		...
		pb_backupbuddy::alert( 'Error: Key files missing.', 'The unzip process reported success but the backup data file, backupbuddy_dat.php was not found in the extracted files. The unzip process either failed (most likely) or the zip file is not a proper BackupBuddy backup.', '9004' );
		return false;
	}
