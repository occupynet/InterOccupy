...
<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo admin_url('admin-ajax.php'); ?>?action=pb_backupbuddy_destination_picker&callback_data=<?php if ( !empty( $_GET['callback_data'] ) ) { echo $_GET['callback_data']; } ?>#pb_backupbuddy_remote_destinations_tab_s3">
	<div class="tablenav">
	...
	<table class="widefat">
		...
		<tbody>
			...
			foreach ( (array)pb_backupbuddy::$options['remote_destinations'] as $destination_id => $destination ) {
				...
				$destination = array_merge( pb_backupbuddy::settings( 's3_defaults' ), $destination );
				?>
				<tr class="entry-row alternate">
					...
					<td>
						<?php echo $destination['title']; ?><br />
							...
							<a href="<?php echo admin_url('admin-ajax.php') . '?action=pb_backupbuddy_destination_picker&edit=' . htmlentities( $destination_id ); ?>&callback_data=<?php if ( !empty( $_GET['callback_data'] ) ) { echo $_GET['callback_data']; } ?>&type=s3#pb_backupbuddy_remote_destinations_tab_s3"><?php _e('Edit Settings', 'it-l10n-backupbuddy' );?></a>
					</td>
...
if ( isset( $_GET['edit'] ) && ( $_GET['edit'] != '' ) && ( $_GET['type'] == 's3' ) ) {
	$options = array_merge( pb_backupbuddy::settings( 's3_defaults' ), (array)pb_backupbuddy::$options['remote_destinations'][$_GET['edit']] );
	
	echo '<h3>', __('Edit Destination', 'it-l10n-backupbuddy' ), '</h3>';
	echo '<form method="post" action="' . admin_url('admin-ajax.php') . '?action=pb_backupbuddy_destination_picker&edit=' . htmlentities( $edit_id ) . '&callback_data=' . $callback_data . '&type=s3#pb_backupbuddy_remote_destinations_tab_s3">';
	echo '	<input type="hidden" name="savepoint" value="remote_destinations#' . $_GET['edit'] . '" />';
} else {
	$options = pb_backupbuddy::settings( 's3_defaults' );
	
	echo '<h3>', __('Add New Destination ', 'it-l10n-backupbuddy' ) . pb_backupbuddy::video( 'njT1ExMgUrk', __('Add a new Amazon S3 destination', 'it-l10n-backupbuddy' ), false ) . '</h3>';
	echo '<form method="post" action="' . admin_url('admin-ajax.php') . '?action=pb_backupbuddy_destination_picker&callback_data=' . $callback_data . '&type=s3#pb_backupbuddy_remote_destinations_tab_s3">';
}
?>
	<input type="hidden" name="#type" value="s3" />
	<input type="hidden" name="required_fields" value="title,accesskey,secretkey,bucket,archive_limit">
	<table class="form-table">
		...
		<tr>
			<td><label for="accesskey"><?php _e('AWS Access Key', 'it-l10n-backupbuddy' ); pb_backupbuddy::tip( __('[Example: BSEGHGSDEUOXSQOPGSBE] - Log in to your Amazon S3 AWS Account and navigate to Account: Access Credentials: Security Credentials.', 'it-l10n-backupbuddy' ) ); ?></label></td>
			<td><input type="text" name="#accesskey" id="accesskey" size="45" maxlength="45" value="<?php echo $options['accesskey']; ?>" /></td>
		</tr>
		<tr>
			<td><label for="secretkey"><?php _e('AWS Secret Key', 'it-l10n-backupbuddy' ); pb_backupbuddy::tip( __('[Example: GHOIDDWE56SDSAZXMOPR] - Log in to your Amazon S3 AWS Account and navigate to Account: Access Credentials: Security Credentials.', 'it-l10n-backupbuddy' ) ); echo ' '; pb_backupbuddy::video( 'Tp_VkLoBEpw', __('Find your Amazon S3 key', 'it-l10n-backupbuddy' ) ); ?></label></td>
		...
		<tr>
			<td><label for="bucket"><?php _e('Bucket Name', 'it-l10n-backupbuddy' ); pb_backupbuddy::tip( __('[Example: wordpress_backups] - This bucket will be created for you automatically if it does not already exist. Bucket names must be globally unique amongst all Amazon S3 users.', 'it-l10n-backupbuddy' ) ); ?></label></td>
			<td><input type="text" name="#bucket" id="bucket" size="45" maxlength="45" value="<?php echo $options['bucket']; ?>" /></td>
		</tr>
		<tr>
			<td><label for="directory"><?php _e('Directory (optional)', 'it-l10n-backupbuddy' ); pb_backupbuddy::tip( __('[Example: backupbuddy] - Directory name to place the backup within.', 'it-l10n-backupbuddy' ) ); echo ' '; pb_backupbuddy::video( 'njT1ExMgUrk#20', __('Create an Amazon S3 bucket', 'it-l10n-backupbuddy' ) ); ?></label></td>
		...
		<tr>
			<td>&nbsp;</td>
			<td>
				<a class="button button-secondary pb_backupbuddy_remotetest" id="pb_backupbuddy_remotetest_s3" alt="<?php echo pb_backupbuddy::ajax_url( 'remote_test' ); ?>&service=s3"><?php _e('Test these settings', 'it-l10n-backupbuddy' );?></a><span class="pb_backupbuddy_loading" style="display: none; margin-left: 10px;"><img src="<?php echo pb_backupbuddy::plugin_url();; ?>/images/loading.gif" <?php echo 'alt="', __('Loading...', 'it-l10n-backupbuddy' ),'" title="',__('Loading...', 'it-l10n-backupbuddy' ),'"';?> width="16" height="16" style="vertical-align: -3px;" /></span>
			</td>
		</tr>
