...
<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo admin_url('admin-ajax.php'); ?>?action=pb_backupbuddy_destination_picker&callback_data=<?php if ( !empty( $_GET['callback_data'] ) ) { echo $_GET['callback_data']; } ?>">
	<div class="tablenav">
	...
	<table class="widefat">
		...
		<tbody>
			...
			foreach ( pb_backupbuddy::$options['remote_destinations'] as $destination_id => $destination ) {
				...
				$destination = array_merge( pb_backupbuddy::settings( 'email_defaults' ), $destination );
				...
				<tr class="entry-row alternate">
					...
					<td>
						<?php echo $destination['title']; ?><br />
							<a href="<?php echo $destination_id; ?>" alt="<?php echo $destination['title'] . ' (' . __('Email', 'it-l10n-backupbuddy' ) . ')'; ?>" class="pb_backupbuddy_selectdestination">
							<?php _e('Select this destination', 'it-l10n-backupbuddy' ); ?>
							</a> | <a href="<?php echo admin_url('admin-ajax.php') . '?action=pb_backupbuddy_destination_picker&edit=' . htmlentities( $destination_id ); ?>&callback_data=<?php if ( !empty( $_GET['callback_data'] ) ) { echo $_GET['callback_data']; } ?>&type=email#pb_backupbuddy_remote_destinations_tab_email"><?php _e('Edit Settings', 'it-l10n-backupbuddy' );?></a>
					</td>
...
?>
	...
	<table class="form-table">
		...
		<tr>
			<td><label for="email"><?php _e('Email', 'it-l10n-backupbuddy' ); pb_backupbuddy::tip( __('[Example: your@email.com] - Email address for this destination.', 'it-l10n-backupbuddy' ) ); ?></label></td>
