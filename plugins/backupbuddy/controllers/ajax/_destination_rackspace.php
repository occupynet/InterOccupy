...
<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo admin_url('admin-ajax.php'); ?>?action=pb_backupbuddy_destination_picker&callback_data=<?php if ( !empty( $_GET['callback_data'] ) ) { echo $_GET['callback_data']; } ?>#pb_backupbuddy_remote_destinations_tab_rackspace">
	<div class="tablenav">
	...
	<table class="widefat">
		...
		<tbody>
			...
			foreach ( (array)pb_backupbuddy::$options['remote_destinations'] as $destination_id => $destination ) {
				...
				$destination = array_merge( pb_backupbuddy::settings( 'rackspace_defaults' ), $destination );
				?>
				<tr class="entry-row alternate">
					...
					<td>
						<?php echo $destination['title']; ?><br />
							...
							<a href="<?php echo admin_url('admin-ajax.php') . '?action=pb_backupbuddy_destination_picker&edit=' . htmlentities( $destination_id ); ?>&callback_data=<?php if ( !empty( $_GET['callback_data'] ) ) { echo $_GET['callback_data']; } ?>&type=rackspace#pb_backupbuddy_remote_destinations_tab_rackspace"><?php _e('Edit Settings', 'it-l10n-backupbuddy' );?></a>
					</td>
...
if ( isset( $_GET['edit'] ) && ( $_GET['edit'] != '' ) && ( $_GET['type'] == 'rackspace' ) ) {
	$options = array_merge( pb_backupbuddy::settings( 'rackspace_defaults' ), (array)pb_backupbuddy::$options['remote_destinations'][$_GET['edit']] );
	
	echo '<h3>', __('Edit Destination', 'it-l10n-backupbuddy' ), '</h3>';
	echo '<form method="post" action="' . admin_url('admin-ajax.php') . '?action=pb_backupbuddy_destination_picker&edit=' . htmlentities( $edit_id ) . '&callback_data=' . $callback_data . '&type=rackspace#pb_backupbuddy_remote_destinations_tab_rackspace">';
	echo '	<input type="hidden" name="savepoint" value="remote_destinations#' . $_GET['edit'] . '" />';
} else {
	$options = pb_backupbuddy::settings( 'rackspace_defaults' );
	
	echo '<h3>', __('Add New Destination', 'it-l10n-backupbuddy' ) . ' ' . pb_backupbuddy::video( 'lfTs_GtAp1I', __('Add a new Rackspace destination', 'it-l10n-backupbuddy' ), false ) . '</h3>';
	echo '<form method="post" action="' . admin_url('admin-ajax.php') . '?action=pb_backupbuddy_destination_picker&callback_data=' . $callback_data . '&type=rackspace#pb_backupbuddy_remote_destinations_tab_rackspace">';
}
?>
	<input type="hidden" name="#type" value="rackspace" />
	<input type="hidden" name="required_fields" value="title,username,api_key,container,archive_limit">
	<table class="form-table">
		...
		<tr>
			<td><label for="username"><?php _e('Username', 'it-l10n-backupbuddy' ); pb_backupbuddy::tip( __('[Example: badger] - Your Rackspace Cloudfiles username.', 'it-l10n-backupbuddy' ) ); ?></label></td>
			<td><input type="text" name="#username" id="username" size="45" maxlength="45" value="<?php echo $options['username']; ?>" /></td>
		</tr>
		<tr>
			<td><label for="api_key"><?php _e('API Key', 'it-l10n-backupbuddy' ); pb_backupbuddy::tip( __('[Example: 9032jk09jkdspo9sd32jds9swd039dwe] - Log in to your Rackspace Cloudfiles Account and navigate to Your Account: API Access', 'it-l10n-backupbuddy' ) ); echo ' '; pb_backupbuddy::video( 'lfTs_GtAp1I#14s', __('Get your Rackspace Cloudfiles API key', 'it-l10n-backupbuddy' ) ); ?></label></td>
			<td><input type="password" name="#api_key" id="api_key" size="45" maxlength="45" value="<?php echo $options['api_key']; ?>" /></td>
		</tr>
		<tr>
			<td><label for="container"><?php _e('Container', 'it-l10n-backupbuddy' ); pb_backupbuddy::tip( __('[Example: wordpress_backups] - This container will NOT be created for you automatically if it does not already exist. Please create it first.', 'it-l10n-backupbuddy' ) ); echo ' '; pb_backupbuddy::video( 'lfTs_GtAp1I#26', __('Create a container from the Rackspace Cloudfiles panel', 'it-l10n-backupbuddy' ) ); ?></label></td>
		...
		<tr>
			<td>&nbsp;</td>
			<td>
				<a class="button button-secondary pb_backupbuddy_remotetest" id="pb_backupbuddy_remotetest_rackspace" alt="<?php echo pb_backupbuddy::ajax_url( 'remote_test' ); ?>&service=rackspace"><?php _e('Test these settings', 'it-l10n-backupbuddy' );?></a><span class="pb_backupbuddy_loading" style="display: none; margin-left: 10px;"><img src="<?php echo pb_backupbuddy::plugin_url();; ?>/images/loading.gif" <?php echo 'alt="', __('Loading...', 'it-l10n-backupbuddy' ),'" title="',__('Loading...', 'it-l10n-backupbuddy' ),'"';?> width="16" height="16" style="vertical-align: -3px;"></span>
			</td>
		</tr>
	</table>
