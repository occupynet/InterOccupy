	...
	class pluginbuddy_zbzipexec extends pluginbuddy_zbzipcore {
			...
			if ( function_exists( 'exec' ) ) {
				...
				if ( $mode == 'unzip' ) {
					...
					if ( file_exists( ABSPATH . 'unzip.exe' ) ) {
						...
						pb_backupbuddy::status( 'details', __('Exec test FAILED: unzip.exe not found at: ','it-l10n-backupbuddy' ) . ABSPATH );
						$result = false;
						return $result;
				...
				if ( file_exists( ABSPATH . 'zip.exe' ) ) {
					...
					@exec( $command . ' "' . $test_file . '" "' . __FILE__ . '"  2>&1', $exec_output, $exec_exit_code );
					...
					if ( file_exists( $test_file ) ) {
						...
						pb_backupbuddy::status( 'details', __('Exec test PASSED.','it-l10n-backupbuddy' ) );
						$result = true;
						...
						pb_backupbuddy::status( 'details', __('Exec test FAILED: Test zip file not found.','it-l10n-backupbuddy' ) );
						pb_backupbuddy::status( 'details', __('Exec Exit Code: ','it-l10n-backupbuddy' ) . $exec_exit_code );
						$result = false;
					...
					pb_backupbuddy::status( 'details', __('Exec test FAILED: zip.exe not found at: ','it-l10n-backupbuddy' ) . ABSPATH );
					$result = false;	
		...
		protected function create_windows( $zip, $dir, $compression, $excludes, $tempdir, $listmaker ) {
			...
			if ( isset( $this->_method_details[ 'param' ][ 'path' ] ) && !empty( $this->_method_details[ 'param' ][ 'path' ] ) ) {
				...
				pb_backupbuddy::status( 'details', __( 'Using custom zip path: ','it-l10n-backupbuddy' ) . $zippath );
				$command = $zippath . 'zip ';
			...
			pb_backupbuddy::status( 'details', $this->get_method_tag() . __(' command (Windows)','it-l10n-backupbuddy' ) . ': ' . $command );
				...
				if ( !empty( $lines ) ) {
					...
					if ( count( $lines ) > self::MAX_ERROR_LINES_TO_SHOW ) {
						...
						if ( file_exists ( $error_file ) ) {
							...
							pb_backupbuddy::status( 'details', __( 'Zip process reported ','it-l10n-backupbuddy' ) . ( count( $lines ) - self::MAX_ERROR_LINES_TO_SHOW ) . __( ' more errors - please review in: ','it-l10n-backupbuddy' ) . $error_file );
