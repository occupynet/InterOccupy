...
class pb_backupbuddy_textreplacebuddy {
	 ...
	 */
	private function _stringreplace_commandline( $file, $search, $replacement, $regex_condition = '', $regex_replace = '' ) {
		...
		if ( stristr( PHP_OS, 'WIN' ) && !stristr( PHP_OS, 'DARWIN' ) ) { // Running Windows. (not darwin)
			...
			if ( $regex_condition == '' ) { // Normal string replace.
				$command = "sed s/{$search}/{$replacement}/g {$file} > {$file}.tmp";
			} else { // Custom regex conditions.
				$command = "sed -E '/{$regex_condition}/s/{$search}/{$replacement}/g' {$file} > {$file}.tmp";
			}
		}
